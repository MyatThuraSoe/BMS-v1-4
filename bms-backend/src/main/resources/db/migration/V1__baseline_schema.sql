-- BMS baseline schema, generated from the entity classes.
-- Applied by Flyway on first startup; Hibernate runs with ddl-auto=validate after this.
...
CREATE TABLE products (
    ...
    INDEX idx_product_active_deleted (is_active, deleted_at),
    INDEX idx_product_category_active (category_id, is_active, deleted_at),
    INDEX idx_product_low_stock (is_active, deleted_at, stock_quantity, min_stock_level)
);
...
-- Invoice sequence tracking (per-day invoice numbers)
...
CREATE TABLE cash_shifts (
    ...
    updated_at DATETIME,
    INDEX idx_cash_shift_cashier (cashier_id),
    INDEX idx_cash_shift_status (status),
    INDEX idx_cash_shift_cashier_status (cashier_id, status),
    INDEX idx_cash_shift_opening_time (opening_time)
);

-- Google Drive backup settings (singleton)
CREATE TABLE backup_settings (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    is_enabled BOOLEAN NOT NULL DEFAULT FALSE,
    frequency VARCHAR(255) NOT NULL DEFAULT 'WEEKLY',
    custom_cron_expression VARCHAR(255),
...
-- Product price history (product detail page)
